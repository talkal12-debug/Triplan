...
| `npm run data:venues [CC]` | Fetch restaurants, cafés, bars, theatres and viewpoints within 1.6 km of every curated attraction into `data/venues/{cc}.json` (OpenStreetMap via Overpass; needs a network that Overpass accepts, i.e. not a cloud host). |
...
A trip can be a beach holiday instead of a sightseeing trip: relax days are a beach (or hotel and pool) block until mid-afternoon plus at most two stops, every third day is a light outing; beaches within 45 km are borrowed from neighbouring demo cities. Demo cities: Lisbon, Sintra, Porto, Rome, Tivoli, Milan, Tokyo, Kamakura-Hakone.
...
- **Other countries on a cloud host**: the public Overpass servers refuse connections from Vercel, so attractions and venues for cities outside the demo set fall back to Nominatim's category search (`src/lib/providers/nominatim-pois.ts`, paced at one request per second, cached). Overpass is still tried first.
- **Restaurants near every meal**: for the demo destinations the venues ship with the app (`data/venues`, see Data); the public Overpass servers refuse or stall connections from cloud hosting, so the live lookup is only used for other cities. Otherwise one Overpass request per plan fetches restaurants/cafes within 500 m of the stop before each lunch and within 900 m of each hotel for dinner (`src/lib/providers/nearby.ts`). Entries are ranked by how complete their OSM record is (website, opening hours, cuisine, Wikidata) and shown with a map link, plus a Google Maps search for ratings we do not have. Fast food only for the budget level.
- **Full days**: a full day is only "done" with at least four stops that run into the late afternoon (half of that on arrival/departure days); until then it is topped up from the unused places, nearest first, stretching the time and walking budgets a little (walking stays strict for parties with a baby, a stroller, someone 65+ or accessibility needs).
...
| `data/venues/{pt,it,jp}.json` | Restaurants, cafés, bars, theatres and viewpoints around the curated attractions (best 40 food + 25 evening entries per attraction by record completeness) | OpenStreetMap via Overpass (ODbL), `npm run data:venues` |
