...
| 8b | **הרחבה** | Auth.js (Google + magic link) + מיגרציית אורח, תכנון משותף, פרופיל + "כבר ביקרתי", יומן, גלריה, צ'אט AI, Lighthouse 90+ | כל תנאי ה-Definition of Done במסמך המקור |
...
4. **Lighthouse 90+ בכל הקטגוריות** עם MapLibre + Framer Motion + 12 שפות: אפשרי, אבל דורש lazy loading אגרסיבי של המפה ואנימציות. נמדד לראשונה ב-8a (`npm run lighthouse`): נגישות/best-practices/SEO כבר 100, ביצועים 63–85 במובייל מדומה. השיפור לביצועים (פיצול JS של השאלון, דחיית hydration) הוא עבודה ל-8b.
...
| 8b | ⏳ הבא | | | Auth.js, תכנון משותף, פרופיל, יומן, גלריה, צ'אט AI, ביצועים 90+ |
