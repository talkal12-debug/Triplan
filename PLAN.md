...
73. **פריסה (13):** SQLite לא עובד ב-Vercel (מערכת קבצים לקריאה בלבד), ו-Prisma לא יודע להחליף ספק לפי סביבה, לכן הסכימה עברה ל-PostgreSQL בכל מקום. הפיתוח המקומי עובד מול אותו מסד Neon (מחרוזת החיבור ב-`.env`), או בלי מסד בכלל: כל הקריאות למסד עטופות ב-fallback (מטמון מקומות, תיאורים), ותכנון אורח לא נוגע במסד. מה שכן דורש מסד: קישורי שיתוף, חשבונות, תכנון משותף. הבנייה ב-Vercel מריצה `prisma db push --accept-data-loss` ו-`db:seed` (upsert, אידמפוטנטי, ~30 שניות) בכל פריסה. הסיכון: שינוי סכימה שמוחק עמודות יעבור בלי שאלה; לפני שינוי כזה מגבים ב-Neon. `AUTH_DEMO_LOGIN` אסור באתר חי (כניסה בשם כל מייל). branch הייצור: `main`. **תקציב זמן:** הפריסה הראשונה נכשלה ב-`/api/plan` עם timeout של 60 שניות (Vercel Hobby). התיקון: `maxDuration = 120` (Fluid compute מאפשר עד 300), ניתוב + מטבע + מסעדות רצים במקביל ב-`enrichPlan`, שני שרתי Overpass במרוץ עם timeout של 15 שניות במקום 40 בזה אחר זה, תיאורי ויקיפדיה בזמן הבנייה רק למקומות שבתוכנית ועם deadline של 12 שניות (השאר נמשך בפתיחת הטיול), וזמני כל שלב נכתבים ל-`notes` (`timing: ...`) וללוג של Vercel. מקומית: ~18 שניות בצד השרת, מהן 15 המתנה ל-Overpass כשהוא עמוס. **Overpass מ-Vercel:** כל ארבעת השרתים הציבוריים (overpass-api.de, kumi, private.coffee, osm.jp) מסרבים או לא עונים לחיבור מהשרתים של Vercel (fetch failed / timeout), גם ב-GET. לכן מסעדות ומקומות ערב ליעדי הדמו נארזים עם האפליקציה: `npm run data:venues` (מהמחשב שלי, שם Overpass עונה) שומר ב-`data/venues/{cc}.json` את 40 מקומות האוכל ו-25 מקומות הערב המלאים ביותר ברדיוס 1.6 ק"מ מכל אטרקציה מאוצרת; בזמן ריצה `src/lib/data/venues.ts` עונה מהקובץ, ו-Overpass נשאר רק לערים שאינן דמו (best effort, עם הערה ב-notes כשנכשל). מצב 2026-09-06: פורטוגל (1,187 מקומות), יפן (1,786) ואיטליה (777; רומא צפופה ו-Overpass מחזיר לה פחות תוצאות) באוויר. **שאר המדינות באתר החי:** גם האטרקציות שלהן הגיעו מ-Overpass, כלומר בלי תיקון אי אפשר היה לתכנן שם טיול בכלל. הפתרון: `src/lib/providers/nominatim-pois.ts`. Nominatim (שכן עונה מ-Vercel) תומך בחיפוש לפי קטגוריה בתוך תיבה ("[museum]", "[castle]"...) ומחזיר את אותם אובייקטים של OSM עם התגים ב-`extratags`; הם ממופים לצורת האלמנטים של Overpass ועוברים באותו סיווג. 18 שאילתות קטגוריה בקצב שנייה אחת (מדיניות השימוש) ≈ 20 שניות לעיר חדשה, פעם אחת (מטמון ב-DB). אותו דבר למסעדות ולערבים בערים שאינן דמו: עד 10 שאילתות לתוכנית (ארוחות קודם). Overpass מנוסה ראשון רק מחוץ ל-Vercel (`overpassReachable`: ב-Vercel בלי `OVERPASS_URL` פרטי מדלגים ישר ל-Nominatim, אחרת 25 שניות של המתנה סתם). מדידה באתר החי לפריז (עיר חדשה, בלי מטמון): 110 שניות לפני הדילוג, מהן 60 בהקשר (Overpass + Nominatim), 21 בהעשרה ו-29 בתיאורי ויקיפדיה שחרגו מה-deadline בגלל retries של 429; ה-deadline נאכף עכשיו גם בתוך ה-retries, ו-`maxDuration` עלה ל-180. הפעם השנייה לאותה עיר מהירה (מטמון ב-DB).
75. **ימים מלאים (בקשת טל, 2026-09-06):** בסקר של 27 תרחישים (3 יעדים × 3 רמות מאמץ × 3 אורכים) יצאו ימים עם 2–3 מקומות שנגמרו ב-13:00–15:00, בעיקר במאמץ "מעט הליכה", כי תקציב ההליכה נגמר והשלמה מהמאגר חיפשה רק ברדיוס 2.5 ק"מ. עכשיו: יום מלא נחשב מלא רק עם ≥4 מקומות (חצי יום: 2) וכשהביקור האחרון נגמר ב-90 הדקות האחרונות של היום או שהזמן הפעיל ≥85%. עד אז ממלאים מהמאגר ברדיוסים 2.5 → 4 → 6 → 10 ק"מ, עם מתיחה של תקציב הזמן ×1.3 ותקציב ההליכה ×1.5 (לא למשפחות עם תינוק/עגלה, בני 65+ או צרכי נגישות: שם ההליכה נשארת קשיחה). הוולידציה מקבלת את אותה מתיחה (מעבר לה: שגיאה; בין 1.0 ל-1.5 אזהרת "תקציב ההליכה כמעט מלא"). אחרי השינוי: כל הימים המלאים בסקר עם ≥4 מקומות ורובם נגמרים 16:00–18:00; ימי "מעט הליכה" נגמרים 15:00–17:00 בכוונה. בדיקת יחידה קבועה על זה. **שני באגים שנחשפו בדרך:** (א) שלב זמני הנסיעה האמיתיים (OSRM) תזמן כל יום מחדש עם התקציב הרגיל והשמיט מקומות שנוספו (וזו גם הסיבה ש"היום קל מדי" נראה תקוע); עכשיו תזמון מחדש מצמיד את כל הביקורים הקיימים, וביקור מוצמד שנדחק אחרי הסגירה/סוף היום נשאר במקומו עם אזהרה במקום להיעלם. (ב) "היום עמוס מדי" תזמן מחדש עם קיבולת ×0.8 ולכן הוריד שני מקומות מיום שנבנה עם המתיחה; עכשיו הוא מצמיד את השאר ומוריד בדיוק אחד.
86. **מילאנו (בקשת טל, 2026-09-13):** עיר דמו שלישית באיטליה, 36 מקומות אוצרים ב-`scripts/pois/italy.ts` (הדואומו, גלריה, לה סקאלה, טירת ספורצה, הסעודה האחרונה, בררה, נאווילי, רובע האופנה, מוזיאונים, פורטה נואובה, סן סירו, אידרוסקאלו ועוד), נפתרו ב-OpenStreetMap עם `npm run data:pois -- IT <ids>` (43 מאומתים, 51 חלקיים, 1 לא מאומת), תיאורים ותמונות מוויקיפדיה, מסעדות ומקומות ערב מ-Overpass, 3 פריטים עונתיים (שבוע העיצוב, שבוע האופנה, Oh Bej), תבנית "סוף שבוע במילאנו". נוסף גם חוף לידו די אוסטיה לרומא (בשביל מצב בטן-גב). שים לב: `data:pois` מוחק את תמונות הערים עד שמריצים `data:summaries IT --images`.
85. **סגנון "בטן-גב" (בקשת טל, 2026-09-13):** `prefs.tripStyle` (explore / relax), שאלה בראש שלב הקצב. במצב relax: כל יום שאינו יום טיול מקבל בלוק `leisure` מהבוקר עד 15:00 בחוף הקרוב (סבב בין החופים ברדיוס 45 ק"מ, חופים אמיתיים לפני מקומות שרק מתויגים "חופים"), או "מנוחה במלון ובבריכה" כשאין חוף; אחריו לכל היותר 2 תחנות; ארוחת הצהריים בחוף. כל יום שלישי מלא (אינדקס 2, 5, 8) הוא יום טיול רגיל אך קל: 3–4 תחנות עם הסיבה "יום טיול בתוך החופשה". ימי הגעה/יציאה הם ימי מנוחה. חופים יוצאים ממאגר האטרקציות. חופים של ערי דמו שכנות מושאלים (החופים של ליסבון רשומים תחת סינטרה) ומתויגים לעיר הקרובה (`borrowBeaches`, `notes: relax: borrowed N beaches`). אין אזהרת "יום קל מדי" כשיש בלוק חוף. כרטיס עם מטרייה, תמונה, "חוף: X"; נושא היום "בטן-גב"; החוף מופיע במפה. תבנית "בטן-גב בליסבון".
84. **פרופילים משפחתיים (בקשת טל, 2026-09-08):** שלב "מי נוסע?" הוא עכשיו שורה לכל אדם: סוג (מבוגר / 65+ / ילד / תינוק), שם (לא חובה), גיל לילדים, ומה חשוב לו (הליכה מועטה, כיסא גלגלים, עגלה, מדרגות, גובה, מנוחת צהריים; רק מה שרלוונטי לסוג). `prefs.travelers`; `party` ו-`accessibility` נגזרים מהשורות (`partyFromTravelers`, `accessibilityFromTravelers`), כך שהמנוע לא השתנה וטיוטות ישנות עם ספירות בלבד הופכות לשורות בפתיחה (`travelersFromParty`). "הליכה מועטה" מפעילה את מצב העדינות במתכנן כמו 65+. שמירה במכשיר (`triplan:family`) וטעינה בטיול הבא. בכל יום בתוכנית שורת "למי היום מתאים": "מתאים לכולם" או "כבד לסבא: הרבה הליכה, יום מאומץ" לפי כללים פשוטים ומוצהרים (`src/lib/trip/day-fit.ts`: מעל 5 ק"מ ל-65+/הליכה מועטה/קטנים, יום מאומץ ל-65+, יום ארוך בלי הפסקה לקטנים, תג wheelchair של המקום לכיסא/עגלה). לא שיפוט רפואי.
83. **מפת "איפה לישון" (2026-09-08):** לכל שהות עד 3 אזורים, מועמדים = מרכז הכובד של כל יום, של כל השהות, והבסיס שהמתכנן בחר; ציון = ממוצע המרחקים למרכז כל יום + חצי מהמרחק הגרוע (בסיס בין שני אזורים מנצח בסיס שמשרת יום אחד); איחוד מועמדים במרחק פחות מ-800 מ'; עוגן = המקום המבוקר הקרוב ("ליד X"); "במרחק הליכה ב-N מתוך M ימים" (2 ק"מ). מחושב בשרת ב-`buildPlanLinks` (`links.sleepZones`, עם קישורי מלונות סביב העוגן דרך בונה הקישורים) כדי שתוכניות שמורות יקבלו אותו ברענון (LINKS_VERSION=7). במפה: עיגולים של 600 מ' עם דירוג, כפתור הצג/הסתר.
82. **"למה דווקא זה" (2026-09-08):** סיבות חדשות מהשעון ב-`timingReasons`: "ראשון בבוקר: נסגר ב-14:00" (סגירה עד 16:00 ואחת משתי התחנות הראשונות), "אחרי הצהריים: נפתח רק ב-14:00", "דווקא היום: סגור ביום שני" (סגור באחד מימי הטיול האחרים), "בסוף היום: תצפית לשקיעה", ו"תחום העניין הראשון שלכם". בכרטיס: 3 צ'יפים + "למה?" שפותח את כל הרשימה. בכותרת היום: "כל התחנות עד N דק' זו מזו", "N מקומות קבועים ליום/לשעה לפי שעות הפתיחה", "צפוי גשם: N מקומות מקורים", "N מקומות מרשימת החובה" (`src/lib/trip/day-why.ts`). תוכניות ישנות מקבלות את הסיבות החדשות רק בבנייה מחדש.
81. **עוד אתרי טיסות (בקשת טל, 2026-09-07, "למה רק Kiwi?"):** בטיול ללונדון הופיע רק Kiwi כי Skyscanner דורש קודי שדה תעופה, ועיר המוצא שהוקלדה לא נמצאה ברשימה (השמות באנגלית בלבד). עכשיו עיר המוצא נפתרת גם בכל שפה: התאמת שם, ואם אין, גיאוקוד ב-Nominatim ושדה התעופה הגדול הקרוב (`src/lib/server/origin-airport.ts`; `buildPlanLinks` הפך אסינכרוני). נוספו Google Flights (משפט חופשי, גם בלי קודים, בלי תוכנית שותפים), Kayak ו-Momondo (`/ORIG-DEST/date/date/Nadults`), ו-Trip.com (קוד עיר: LON ולא LHR, מפת ערים מרובות-שדות ב-`metroCode`). כל הפורמטים אומתו בדפדפן עם תוצאות אמיתיות (TLV–LHR). LINKS_VERSION=6 כדי שתוכניות קיימות יקבלו את הקישורים בפתיחה.
80. **סוגי אירועים לערב (בקשת טל, 2026-09-07, אחרי שהמפתח של Ticketmaster נכנס):** בשלב תחומי העניין, מתחת ל"סגנון ערב", שאלה עם בחירה מרובה: קונצרט, מחזמר, תיאטרון, סטנדאפ, קלאסי ואופרה, מחול, ספורט, משפחה (`prefs.eventTypes`, ריק = הכול; מוסתר כשבחרו "בלי ערבים"). בבניית התוכנית רצות שתי שאילתות ל-Ticketmaster לכל שהות: אחת מוגבלת לסוגים שנבחרו (`classificationName`) ואחת כללית, מאוחדות בלי כפילויות; ההתאמה לסוג נעשית אצלנו לפי הסיווג של המוכר (segment/genre/subGenre, מילות מפתח ב-`src/lib/nearby/event-types.ts`), המועדפים עולים ראשונים ומסומנים "לפי ההעדפות שלכם"; השאר נשארים אחריהם, לא נמחקים. שמות הסיווג של Ticketmaster (Music, Musical, Theatre, Comedy, Classical, Opera, Dance, Sports, Family, Children) הם לפי התיעוד שלהם ולא אומתו מול ה-API בגלל שאין לי מפתח מקומי; אם שם לא קיים, השאילתה המוגבלת מחזירה ריק והכללית עדיין עונה.
79. **באג אזור זמן (נמצא 2026-09-07 בבדיקת Ticketmaster באתר החי):** next-intl יורש את אזור הזמן של השרת (UTC ב-Vercel) ומעביר אותו ללקוח; תאריכי לוח ("2026-10-02") שנבנו כחצות מקומית הוצגו יום אחד מוקדם לכל מי שמזרחה מגריניץ' (ישראל), בסיכום השאלון, בלשוניות הימים, בלוח השנה (גם שמות חודשים וימים), בצ'ק-ליסט, בהדפסה ובשיתוף. מקומית לא נראה כי שרת הפיתוח רץ בשעון ישראל. הפתרון: `useCalendarFormat` בונה את היום בחצות UTC ומעצב ב-UTC; לוח השנה עושה את כל החשבון ב-UTC; שעות אירועים (Ticketmaster) נשמרות כשעת המקום ומודפסות כמו שהן. כלל: תאריך לוח לעולם לא עובר דרך `new Date("YYYY-MM-DDT00:00:00")` לתצוגה.
78. **סבב שני (בקשת טל, אותו יום):** רקע לבן במקום קרם, כחול עמוק במקום ירוק, גופן אחד (Heebo) גם לכותרות (Frank Ruhl Libre הוסר, "מיושן"), ותמונות: לכל מקום עם ערך בוויקיפדיה נשמרת תמונת הפתיח של הערך (`summary[lang].image`, מ-REST `thumbnail`), ולכל עיר דמו תמונת הערך שלה (`city.image`); `npm run data:summaries --images` ממלא (48/86 פורטוגל, 41/58 איטליה, 40/63 יפן, 7/7 ערים). מוצג: רצועת תמונות בדף הבית, תמונת כותרת לכל תבנית בגלריה, אריחי ערים בשאלון, תמונה ממוזערת בכל כרטיס אטרקציה, באנר בתצוגת "עכשיו". קרדיט "תמונה: ויקיפדיה" עם קישור לערך. **תקלה שנתגלתה:** מ-2025 ויקימדיה מגישה לאנונימיים רק רוחבי תמונה קבועים (250/330/500/960/1280/1920), רוחב אחר מחזיר 400; `wikiThumb` מעגל לרוחב הקרוב. תוכניות ישנות מקבלות תמונות בפתיחה הבאה (סמן `he@2`).
77. **עיצוב מחדש (בקשת טל, 2026-09-07): "פחות קלודי".** מה הוחלף: הפלטה (טורקיז + כתום על קרם) לדיו על פשתן עם ירוק אורן עמוק כצבע חזק יחיד ופליז לקווי שיער וסימונים קטנים (dark: פחם חם עם מרווה); הגופנים (Rubik) ל-Heebo לגוף ו-Frank Ruhl Libre (סריף עברי-לטיני) לכותרות ולספרות; רדיוס 12px→6px בכל הרכיבים (55 rounded-2xl, 33 rounded-xl הוחלפו); הגרדיאנט והכתמים הזוהרים בדף הבית לפריסה עריכתית: כותרת סריף גדולה, "eyebrow" עם קו פליז, שלושת השלבים כתוכן עניינים עם ספרות סריף, יתרונות בשלוש עמודות עם מפרידי קו-שיער ובלי כרטיסים; ניווט עליון עם קו תחתון בפליז במקום גלולות; לשוניות עם קו תחתון; כפתורים ריבועיים עם tracking; צ'יפים בשאלון מרובעים. עקרונות: צבע אחד חזק, קווי שיער במקום צללים, סריף לכותרות וספרות, בלי גרדיאנטים/זוהר/emoji. כל טקסט ה-UI, המבנה, ה-RTL וה-e2e נשארו כמו שהיו.
76. **אירועים עם כרטיסים (בקשת טל):** אין API לאירועים שעובד בלי מפתח (בדקתי: Ticketmaster, SeatGeek, Songkick, Eventbrite, PredictHQ, Bandsintown — כולם דורשים רישום). ה-Ticketmaster Discovery API חינמי אחרי רישום של 5 דקות (`docs/deploy.md`), והקוד מוכן: אירועים בתאריך, טווח מחירים, תמונה, וכפתור "לרכישת כרטיסים" (עם `rel=sponsored`; עמלה דרך Impact כשמגדירים `AFFILIATE_QUERY_TICKETMASTER`). נוסף בלוק "אירועים והופעות בתאריכי הטיול" מעל התוכנית שמרכז את כל האירועים של כל הימים. **לא נבדק מול ה-API האמיתי** עד שטל ייצור מפתח.
74. **"היום קל מדי" שלא הגיב (בקשת טל):** הכפתור בחר את המקום הפנוי הטוב ביותר ברדיוס 3 ק"מ מהתחנה האחרונה ושלח לתזמון; כשהמקום לא נכנס (תקציב הליכה, שעות פתיחה) היום חזר זהה בלי הסבר. עכשיו: עד 8 מועמדים ברדיוס 4 ק"מ מהתחנה האחרונה או מהבסיס, הראשון ששורד את התזמון נכנס; וכשבאמת אין מה להוסיף/להוריד, הודעה "לא היה מה לשנות" במקום שתיקה (`plan.editNoChange`).
